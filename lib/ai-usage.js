...
/** 점성학 등 긴 해석: max_tokens 도달 시 1회 이어쓰기 (추가 크레딧 없음) */
async function callAnthropicWithOptionalContinue({ model, max_tokens, messages, feature }) {
  const cap = feature === 'astro' ? 8192 : Math.min(Number(max_tokens) || 1000, 4000);
  const tokens = Math.min(Number(max_tokens) || 1000, cap);
  let { status, data } = await callAnthropic({ model, max_tokens: tokens, messages });
  if (status < 200 || status >= 300) return { status, data };
  if (feature !== 'astro' || data?.stop_reason !== 'max_tokens') return { status, data };

  const firstText = anthropicText(data);
  const contMessages = [
    ...messages,
    { role: 'assistant', content: firstText },
    {
      role: 'user',
      content: '위 해석이 출력 한도로 중간에 끊겼습니다. 끊긴 문장 바로 다음부터 이어서, 아직 작성하지 않은 ## 섹션만 모두 완성해 주세요. 이미 쓴 섹션과 내용은 반복하지 마세요.',
    },
  ];
  const cont = await callAnthropic({ model, max_tokens: tokens, messages: contMessages });
  if (cont.status < 200 || cont.status >= 300) return { status, data };

  const mergedText = firstText + (anthropicText(cont.data) ? `\n\n${anthropicText(cont.data)}` : '');
  return {
    status: 200,
    data: {
      ...cont.data,
      stop_reason: cont.data?.stop_reason === 'max_tokens' ? 'max_tokens' : 'end_turn',
