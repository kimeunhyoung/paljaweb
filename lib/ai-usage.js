...
/** 체험권은 월 25크레딧, 유료 Professional은 120. 구매 보너스는 플랜 한도에 합산. */
function aiCreditLimit(profile) {
  const bonus = purchasedBonus(profile);
  if (isCounselorTrialProfile(profile)) return COUNSELOR_TRIAL_AI_LIMIT + bonus;
  return planLimit(effectivePlan(profile)) + bonus;
}
...
function computeAiRemaining(profile, monthlyUsed) {
  const plan = effectivePlan(profile);
  const planCap = isCounselorTrialProfile(profile)
