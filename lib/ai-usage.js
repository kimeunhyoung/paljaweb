...
/** 점성학 AI: 요청당 최대 출력 + 끊기면 이어쓰기 최대 횟수 */
const ASTRO_MAX_OUTPUT_TOKENS = 16384;
const ASTRO_MAX_CONTINUE_ROUNDS = 6;
...
async function callAnthropicWithOptionalContinue({ model, max_tokens, messages, feature }) {
  const isAstro = feature === 'astro';
  const cap = isAstro ? ASTRO_MAX_OUTPUT_TOKENS : Math.min(Number(max_tokens) || 1000, 4000);
  const tokens = Math.min(Number(max_tokens) || 1000, cap);
  const maxRounds = isAstro ? ASTRO_MAX_CONTINUE_ROUNDS : 1;
  ...
  for (let round = 0; round < maxRounds; round += 1) {
    ...
    if (!isAstro || lastData?.stop_reason !== 'max_tokens') {
      return {
        status: 200,
        data: {
          ...lastData,
