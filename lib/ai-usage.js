...
const PLAN_RANK = { free: 0, basic: 1, pro: 2, professional: 3, special: 4, professional2: 4 };

function normalizeDbPlan(plan) {
  return plan === 'professional2' ? 'special' : (plan || 'free');
}
...
async function callAnthropicWithOptionalContinue({ model, max_tokens, messages, feature }) {
  const isAstroChart = feature === 'astro';
...
function registerAiUsageRoutes(app, { getUserIdFromAuth, getProfile, isAiAvailable }) {
  ...
  app.post('/api/ai/messages', async (req, res) => {
    ...
    if (!feature || !FEATURES[feature]) {
      res.status(400).json({
        error: 'feature가 필요합니다. (astro | astro_transit | astro_year | numerology_daily | numerology_monthly | name_opinion | name_recommend)',
      });
      return;
    }
