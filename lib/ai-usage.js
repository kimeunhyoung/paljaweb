...
const PLAN_RANK = { free: 0, basic: 1, pro: 2, plus: 2, professional: 3, special: 4, private: 4, professional2: 4 };
...
function assertFeatureAccess(profile, feature) {
  ...
  if ((PLAN_RANK[plan] || 0) < (PLAN_RANK[meta.minPlan] || 0)) {
    return {
      ok: false,
      status: 403,
      error: `${meta.label}은(는) ${meta.minPlan === 'pro' ? 'Plus' : 'Basic'} 이상 플랜에서 이용할 수 있어요.`,
      code: 'plan_required',
