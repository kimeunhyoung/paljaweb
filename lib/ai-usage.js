...
const FEATURES = {
  ...
  counselor_session: { cost: 2, minPlan: 'professional', label: '상담사 AI 심화 리딩 (1~3장)' },
};
...
async function callAnthropicWithOptionalContinue({ model, max_tokens, messages, feature, temperature }) {
  ...
  const isCounselor = feature === 'counselor_session' || feature === 'counselor_session_basic';
...
function registerAiUsageRoutes(app, { getUserIdFromAuth, getProfile, isAiAvailable }) {
  ...
  app.post('/api/ai/messages', async (req, res) => {
    ...
    if (!feature || !FEATURES[feature]) {
      res.status(400).json({
        error: 'feature가 필요합니다. (astro | astro_couple | astro_transit | astro_year | numerology_daily | numerology_monthly | name_opinion | name_recommend | counselor_session_basic | counselor_session)',
      });
      return;
    }
    ...
    try {
      const isCounselor =
        feature === 'counselor_session' || feature === 'counselor_session_basic';
