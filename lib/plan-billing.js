...
const PLAN_TIER_ORDER = { free: 0, basic: 1, pro: 2, professional: 3, professional2: 4 };
...
function isCounselorPlan(planOrProfile) {
  const p =
    typeof planOrProfile === 'string' ? planOrProfile : effectivePlan(planOrProfile);
  return p === 'professional' || p === 'professional2';
}

function planTierRank(plan) {
  return Object.prototype.hasOwnProperty.call(PLAN_TIER_ORDER, plan)
    ? PLAN_TIER_ORDER[plan]
    : -1;
}
...
function computeUpgradeAmount(profile, targetPlan, now = new Date()) {
  ...
  if (!PLAN_AMOUNTS[targetPlan]) return { ok: false, reason: 'invalid_target' };
  if (planTierRank(targetPlan) <= planTierRank(fromPlan)) return { ok: false, reason: 'not_upgrade' };
  ...
  const toAmount = PLAN_AMOUNTS[targetPlan]?.[cycle];
