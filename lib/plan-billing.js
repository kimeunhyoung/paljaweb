/** 출시 기념 할인가 (정식가: Plus 29900 / Professional 49900) */
const PLAN_AMOUNTS = {
  basic: { monthly: 9900, annual: 7920 * 12 },
  plus: { monthly: 19900, annual: 15920 * 12 },
  professional: { monthly: 29900, annual: 23920 * 12 },
};

/** 요금제 페이지·마케팅용 정식가 (표시용) */
const PLAN_LIST_PRICES = {
  basic: { monthly: 9900 },
  plus: { monthly: 29900 },
  professional: { monthly: 49900 },
};
...
/** 갱신 청구액: 가입 시 고정가 우선, 없으면 현재 요금표 */
function resolveSubscriptionAmount(profile, plan, cycle) {
  const locked = Number(profile?.subscription_amount);
  if (Number.isFinite(locked) && locked > 0) return locked;
  const cfg = PLAN_AMOUNTS[normalizePlanSlug(plan)];
  if (!cfg) return null;
  return cycle === 'annual' ? cfg.annual : cfg.monthly;
}

/** 신규·업그레이드 시 잠글 금액 (항상 현재 요금표) */
function listAmountForPlan(plan, cycle) {
  const cfg = PLAN_AMOUNTS[normalizePlanSlug(plan)];
  if (!cfg) return null;
  return cycle === 'annual' ? cfg.annual : cfg.monthly;
}
...
function computeUpgradeAmount(profile, targetPlan, now = new Date()) {
  ...
  const fromAmount =
    resolveSubscriptionAmount(profile, fromPlan, cycle) || PLAN_AMOUNTS[fromPlan]?.[cycle];
  const toAmount = listAmountForPlan(normalizedTarget, cycle);
...
function createPlanBilling(deps) {
  ...
  async function extendPlanForUser(userId, plan, cycle, paymentKey, extra = {}) {
    ...
    const lockedAmount =
      extra.subscription_amount != null
        ? Number(extra.subscription_amount)
        : listAmountForPlan(normalizedPlan, cycle);
    const fields = {
      plan: normalizedPlan,
    ...
    if (Number.isFinite(lockedAmount) && lockedAmount > 0) {
      fields.subscription_amount = lockedAmount;
    }
    await patchProfileFields(userId, fields);
    return until;
  }

  async function activateSubscription(userId, plan, cycle, paymentKey, billingKey, amount) {
    const locked =
      amount != null && Number.isFinite(Number(amount)) && Number(amount) > 0
        ? Number(amount)
        : listAmountForPlan(plan, cycle);
    return extendPlanForUser(userId, plan, cycle, paymentKey, {
      toss_billing_key: billingKey,
      subscription_cancel_at_period_end: false,
      basic_pass_license_id: null,
      counselor_trial_license_id: null,
      subscription_amount: locked,
    });
  }

  /** confirm과 웹훅이 겹쳐도 같은 paymentKey로 기간을 한 번만 연장 */
  async function attachBillingKeyIfMissing(userId, billingKey) {
    if (!billingKey) return false;
    const profile = await getProfile(userId);
    if (profile?.toss_billing_key) return false;
  ...
  async function activateSubscriptionOnce(userId, plan, cycle, paymentKey, billingKey, amount) {
    const claimed = await insertAppliedPaymentKey(paymentKey, userId, paymentKey);
    if (!claimed) {
      await attachBillingKeyIfMissing(userId, billingKey);
      return { ok: true, skipped: true, plan: normalizePlanSlug(plan) };
    }
    await activateSubscription(userId, plan, cycle, paymentKey, billingKey, amount);
    return { ok: true, skipped: false, plan: normalizePlanSlug(plan) };
  }
  ...
  // 이후 갱신 청구가는 새 플랜의 현재 요금표로 고정.
  async function applyPlanUpgrade(userId, targetPlan, paymentKey, cycle = 'monthly') {
    const normalizedTarget = normalizePlanSlug(targetPlan);
    const locked = listAmountForPlan(normalizedTarget, cycle);
    await patchProfileFields(userId, {
      plan: normalizedTarget,
      professional_payment_key: paymentKey,
      subscription_cancel_at_period_end: false,
      ...(Number.isFinite(locked) && locked > 0 ? { subscription_amount: locked } : {}),
    });
  }
  ...
  async function applyPlanPaymentSuccess(paymentKey, orderId, totalAmount, ctx) {
    ...
    await extendPlanForUser(userId, plan, cycle, paymentKey, {
      subscription_amount: Number.isFinite(amt) && amt > 0 ? amt : listAmountForPlan(plan, cycle),
    });
    return { ok: true, plan };
  }
  ...
  return {
    ...
    activateSubscriptionOnce,
    attachBillingKeyIfMissing,
