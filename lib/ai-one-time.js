...
/**
 * @typedef {{ key: string, amount: number, credits: number, passDays?: number, orderName: string, title: string, desc: string, scope?: string }} AiOneTimeProduct
...
const AI_ONETIME_PRODUCTS = {
  ai_chart: {
    ...
    credits: 2,
    scope: 'astrology',
    orderName: '점성학 AI 차트 해석 1회',
    title: '점성학 AI 차트 해석 1회',
  ...
  ai_fortune: {
    ...
    title: '점성학 AI 운세 체험팩',
    desc: '점성학용 AI 6크레딧 · 2,500원(단건 1,000원×3=3,000원보다 이득). 예: 차트 해석(2) + 올해 운세(1) + 이번 주 운세(1) + 오늘 운세(1) + 여유 1.',
  },
  cal_pass_3d: {
    key: 'cal_pass_3d',
    amount: 1900,
    ...
    title: '수비학 달력 3일 체험',
    desc: '수비학 달력 3일 이용 + AI 3크레딧. 월간 달력·일별 가이드·AI 오늘 운세/이번 달 흐름을 짧게 체험. 계속 쓰면 Basic 월 9,900원이 이득입니다.',
  },
};
...
function isAiCreditPaymentId(paymentId) {
  ...
  return s.startsWith('aich') || s.startsWith('aifr') || s.startsWith('acal');
}
