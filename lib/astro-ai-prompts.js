...
const DIAG_STYLE =
  ...
  '2) 위로·자기계발 칼럼·일반 팁 금지. 「행운」 대신 「구조적 기회·타고난 프리패스」, 「운이 나쁘다」 대신 「구조적 취약성」.\n' +
  ...
  '4) Part A 본문에는 「트라인」「육분」「사각」 등 전문 용어 날것 금지(일상어: 프리패스·막힘 없는 통로·마찰 지점). ' +
  '전문 용어·각도 이름은 Part B에만. 이모지 남발·모욕·선동 금지.\n' +
  '5) 4번의 작전 지시는 구체 동작 1개씩. 보장·협박 금지. 「지키면 프리패스가 켜지기 쉽고, 안 지키면 결함 패턴으로 돌아간다」 톤.\n' +
...
function pushDiagPartA(L, name, domainLabel, focusLine, whyBullets) {
  ...
  L.push('');
  L.push(
    '> **[핵심 진단]:** (고질적 결함 한 축 + 절대 빼앗기 어려운 타고난 무기(프리패스) 한 축을 한 줄로)',
  );
  L.push('');
  ...
  L.push(
    '* **왜 여기서 승부해야 하는가:** (이 DOMAIN에서 남들보다 효율이 나는 이유 1~2줄. “엉뚱한 데서만 버티는 게 오답”을 짚되 모욕 금지)',
  );
  L.push('');
  L.push('### 3. [위험] 치명적 결함: 쪽박 시나리오');
  L.push('* **구조적 취약성:** (부스터를 무시하거나 오작동할 때 붕괴하는 패턴. 「운이 나쁘다」 변명 금지)');
  L.push(
  ...
  L.push(
    '* (뻔한 상담 조언 금지. 결함을 막고 **타고난 프리패스**를 켜는 작전 지시. 각 항목은 구체 동작 1개)',
  );
  L.push(
    '* **[Emergency Stop]:** 쪽박 시나리오 징후가 보이면 즉시 멈출 행동 (이 사람에게 맞게)',
  );
  L.push(
    '* **[Level Up]:** 내일부터 프리패스 구간을 쓰기 위한 루틴 1개 (이 사람에게 맞게)',
  );
  L.push(
    '* **[분석가의 한마디]:** 뼈 있는 한 방. 보장·협박·모욕 금지. 「프리패스를 쓰면 구조가 켜지고, 안 쓰면 결함으로 돌아간다」 톤.',
  );
  L.push('');
