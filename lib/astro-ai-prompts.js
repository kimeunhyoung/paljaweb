...
const DIAG_STYLE =
  ...
  '2) 위로·자기계발 칼럼·일반 팁 금지. 「행운」 대신 「덜 버티고 되는 구간」, 「운이 나쁘다」 대신 「유난히 안 되는 구간·고정된 마찰」.\n' +
  '3) **덜 버티고 되는 구간(매우 중요):** [출생 차트]의 **트라인·육분 등 길한 연결**이 있으면 Part A 2번의 핵심으로. ' +
  '“누구나 노력하면 되는 강점”이 아니라 **남들은 애써도 막히는 일을 이 사람은 덜 싸우고 뚫는 타고난 통로**로 설명. ' +
  '길한 각이 약하면 강한 하우스 집중·길한 행성 배치로 대체하되 지어내지 마세요.\n' +
  '4) **유난히 안 되는 구간(매우 중요):** **충·사각 등 긴장 각**이 있으면 Part A 3번의 핵심으로. ' +
  '의지 부족이 아니라 **남들은 평범히 되는 일이 이 사람에게는 두 배로 힘든 고정 마찰**로 설명. 긴장 각이 없으면 실제 취약 배치만.\n' +
  ...
  '7) 4번 작전은 구체 동작 1개씩. 보장·협박·모욕 금지. 「되는 구간을 쓰고, 안 되는 구간을 밀어붙이지 않으면 구조가 켜진다」 톤.\n' +
...
function pushDiagPartA(L, name, domainLabel, focusLine, whyBullets) {
  ...
  L.push('');
  L.push(
    '> **[핵심 진단]:** (덜 버티고 되는 구간 한 축 + 유난히 안 되는 구간 한 축을, 삶의 말로 한 줄)',
  );
  L.push('');
  ...
  L.push('');
  L.push('### 2. 덜 버티고 되는 구간: 남들은 애써도, 너는 덜 싸우고 뚫리는 곳');
  L.push(
    '* **불공평한 타고남:** (길한 연결이 있으면 그걸 핵심으로. 「큰 그림을 잡는 힘이 덜 막힌다」「실행과 확장이 같은 방향이다 등 삶 언어만. 행성쌍·각도명 금지)',
  );
  L.push(
    '* **왜 여기서 승부해야 하는가:** (이 DOMAIN에서 남들보다 효율이 나는 이유 1~2줄. 되는 구간을 안 쓰고 안 되는 구간만 파는 게 오답임을 짚되 모욕 금지)',
  );
  L.push('');
  L.push('### 3. 유난히 안 되는 구간: 밀어붙이면 쪽박 나는 곳');
  L.push(
    '* **고정된 마찰:** (긴장 각이 있으면 그걸 핵심으로. 「감정과 실행이 서로 잡아끈다」「흔들리면 회로가 과열되거나 멈춘다」 등. 행성쌍·각도명 금지. 의지 부족으로 몰지 말 것)',
  );
  L.push(
  ...
  L.push(
    '* (뻔한 상담 조언 금지. **안 되는 구간을 피하고 되는 구간을 켜는** 작전. 각 항목은 구체 동작 1개)',
  );
  L.push(
    '* **[Emergency Stop]:** 쪽박 징후가 보이면 즉시 멈출 행동 (이 사람에게 맞게)',
  );
  L.push(
    '* **[Level Up]:** 내일부터 되는 구간을 쓰기 위한 루틴 1개 (이 사람에게 맞게)',
  );
  L.push(
    '* **[분석가의 한마디]:** 뼈 있는 한 방. 보장·협박·모욕 금지. 「되는 구간을 쓰고 안 되는 구간을 피하면 구조가 켜진다」 톤.',
  );
  L.push('');
...
function buildAstroMoneyPrompt(payload) {
  ...
  pushDiagPartA(
    ...
    name,
    '재물',
    '돈의 유입·누수·베팅 버릇의 설계 오류 / 덜 버티고 되는 구간 / 유난히 안 되는 구간·쪽박 시나리오만. Part A는 일상어만.',
    [
      '### 근거 1 — 설계도',
      '(2·5·8하우스·금성·목성·토성 등 실제로 있는 배치)',
      '### 근거 2 — 덜 버티고 되는 구간(트라인·육분 등 길한 연결)',
      '(행성·각도 이름을 여기서 밝히고 쉬운 말로 풀기)',
      '### 근거 3 — 유난히 안 되는 구간(충·사각 등 긴장 각)과 작전 지시',
      '(행성·각도 이름을 여기서 밝히고 쉬운 말로 풀기)',
    ],
  );
  return L.join('\n');
...
function buildAstroLovePrompt(payload) {
  ...
  pushDiagPartA(
    ...
    name,
    '연애',
    '끌림·선택·어긋남의 설계 오류 / 덜 버티고 되는 구간 / 유난히 안 되는 구간·쪽박 시나리오만. 「운명의 상대」 단정 금지. Part A는 일상어만.',
    [
      '### 근거 1 — 설계도',
...
function buildAstroCareerPrompt(payload) {
  ...
  pushDiagPartA(
    ...
    name,
    '커리어',
    '일 방식의 설계 오류 / 덜 버티고 되는 구간 / 유난히 안 되는 구간·번아웃·정체 쪽박 시나리오만. Part A는 일상어만.',
    [
      '### 근거 1 — 설계도',
...
function buildAstroStudyPrompt(payload) {
  ...
  pushDiagPartA(
    ...
    name,
    '학업·성장',
    '학습 스위치·누수·압박 붕괴의 설계 오류 / 덜 버티고 되는 구간 / 유난히 안 되는 구간·쪽박 시나리오만. Part A는 일상어만.',
    [
      '### 근거 1 — 설계도',
