...
const CHEAT_STYLE =
  '[작성 원칙 — 최우선]\n' +
  '1) 학술 분석·벽돌 줄글 금지. 전부 불릿·짧은 키워드 위주. 스크롤 2~3번이면 끝나게.\n' +
  '2) 출력문에 「6하우스」「트라인」「사각」「충」「어스펙트」「천왕성」 등 전문 용어를 날것 그대로 쓰지 마세요. ' +
    '내부적으로만 차트 데이터를 읽고, 밖으로는 「성공 부스터」「위험 경고등」「관계 부스터」처럼 일상어로만 말하세요.\n' +
  '3) 「어떠어떠한 경향」식 두루뭉술 금지. 반드시 **DO(이렇게 하라)** / **DON\'T(절대 하지 마라)** 행동 지침을 넣으세요.\n' +
  '4) 직설적이고 또렷하되, 「상위 1%」「도파민」 같은 과시·과장 톤은 쓰지 마세요. 이모지 남발 금지(제목 장식용 최소만).\n' +
  '5) [출생 차트]에 있는 배치만 근거. 없는 배치를 지어내지 마세요. 이름은 차트에 있으면 그대로, 없으면 「당신」.';
...
function pushCheatClosing(L) {
  L.push(CHEAT_STYLE);
  L.push('위 [출력 포맷]의 제목·불릿 구조를 **빠짐없이** 채우고 중간에 끊지 마세요.');
}
...
function buildAstroMoneyPrompt(payload) {
  ...
  L.push(
    '당신은 냉철하고 직관적인 재무·점성 전략 파트너입니다. ' +
      '차트에서 돈을 버는·지키는 패턴을 읽어, 피로한 학술 분석이 아니라 「당장 써먹는 돈 치트키 매뉴얼」을 씁니다. ' +
      COMMON_TONE,
  );
  L.push('');
  appendChartBlock(L, p);
  ...
  L.push('* **안전장치:**');
  L.push('  1. (큰 결정 전 반드시 거칠 브레이크 루틴)');
...
function buildAstroLovePrompt(payload) {
  ...
  L.push(
    '당신은 인간관계 패턴을 꿰뚫는 라이프·점성 전략 파트너입니다. ' +
      '학술 분석이 아니라 「당장 써먹는 연애·관계 치트키 매뉴얼」을 씁니다. ' +
      COMMON_TONE,
  );
  L.push('');
  appendChartBlock(L, p);
...
function buildAstroCareerPrompt(payload) {
  ...
  L.push(
    '당신은 커리어 성공 패턴을 읽는 라이프·점성 전략 파트너입니다. ' +
      '수입액·재물보다 「일의 방식·성취·역할」에 초점을 두고, 「당장 써먹는 커리어 치트키 매뉴얼」을 씁니다. ' +
      COMMON_TONE,
  );
  L.push('');
  appendChartBlock(L, p);
...
function buildAstroStudyPrompt(payload) {
  ...
  L.push(
    '당신은 학습·성장 동력을 읽는 라이프·점성 전략 파트너입니다. ' +
      '학술 분석이 아니라 「당장 써먹는 공부·성장 치트키 매뉴얼」을 씁니다. ' +
      COMMON_TONE +
  ...
  L.push('');
  L.push('> **[핵심 한 줄 요약]:** (지식을 흡수하고 실력을 증명하는 학습 본질 한 문장)');
  L.push('');
  L.push('### 1. 당신의 지식 흡수 엔진 (학습 스타일)');
  L.push('* **습득 방식:** (눈으로 / 몸으로 / 스토리로 등)');
  L.push('* **집중력 패턴:** (어떤 환경·타이밍에 뇌가 잘 도는지)');
  L.push('');
  L.push('### 2. 시험·성취 필살기');
  L.push('* **최적의 공부법:** (시간 대비 효율이 높은 맞춤 전략)');
  L.push('* **DO (추천 행동):** (실력 발휘를 위해 지킬 루틴 1~2가지)');
  L.push('* **DON\'T (금지 행동):** (시간만 낭비하는 공부 습관 1~2가지)');
  L.push('');
  L.push('### 3. [경고] 압박과 멘탈 붕괴의 함정');
  L.push('* **위험 패턴:** (중요한 순간에 실력 발휘를 막는 심리적 장벽)');
  L.push('* **안전장치:**');
  L.push('  1. (시험 직전·슬럼프 때 멘탈을 붙잡는 브레이크)');
  L.push('  2. (학습 효율을 위한 일상 에너지 관리)');
  L.push('');
  pushCheatClosing(L);
  return L.join('\n');
}
