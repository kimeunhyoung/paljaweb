...
const DIAG_STYLE =
  ...
  '6) **복이 없거나 긴장이 몰린 경우(매우 중요):** 이 DOMAIN에 마찰 없는 연결이 거의 없거나, 긴장이 극단적으로 몰려 있으면 ' +
  '없는 복을 지어내거나 숨기지 마세요. 2번에서는 「이 영역만큼은 쉽게 되는 타고난 복이 약하다」를 정면으로 쓰고, ' +
...
function pushDiagPartA(L, name, domainLabel, focusLine, whyBullets) {
  ...
  L.push(
    '* (이 DOMAIN에서 정보를 받아들이고 결정할 때, 남들과 다른 회로·기조가 무엇인지. “왜 이 방면에서 다르게 생각하는가”의 근본 성향. 삶 언어만)',
  );
  L.push('');
  ...
  L.push(
    '* (그런 연결이 거의 없으면: 없는 복을 지어내지 말고, 「이 영역만큼은 쉽게 되는 타고난 복이 약하다」를 정면으로. 그 결핍이 DOMAIN에서 어떤 느낌인지)',
  );
  L.push('');
  ...
  L.push(
    '* (긴장이 이 DOMAIN에 몰려 있으면: 그 몰림을 숨기지 말고, 왜 이 영역만큼은 남들처럼 안 되는지 인과로)',
  );
  L.push('');
...
function buildAstroMoneyPrompt(payload) {
  ...
  pushDiagPartA(
    ...
    [
      ...
      '### 근거 2 — 남들보다 타고난 복(트라인·육분 등). 없으면 「이 DOMAIN엔 길한 연결이 약함」을 근거로 명시',
