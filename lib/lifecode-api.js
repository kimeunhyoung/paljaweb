...
const BETA_DAYS_NOTE_RE = /\[beta_days:(\d{1,3})\]/i;

function parseBetaAiFromLicenseNote(note) {
...
/** 시스템 태그·자동 라벨을 제외한 운영 메모 (카페·테스터명 등) */
function extractLicenseMemo(note) {
  let s = String(note || '');
  s = s.replace(BETA_AI_NOTE_RE, '').replace(BETA_DAYS_NOTE_RE, '');
  s = s.replace(/상담사\s*테스터\s*\d{1,3}\s*일\s*·\s*AI\s*\d{1,3}/gi, '');
...
function rebuildLicenseNote({ betaAi, betaDays, memo }) {
  const tags = [];
  if (Number.isInteger(betaAi) && betaAi > 0) {
    tags.push(`[beta_ai:${Math.min(500, betaAi)}]`);
  }
  if (Number.isInteger(betaDays) && betaDays > 0) {
    tags.push(`[beta_days:${betaDays}]`);
  }
  const chunks = [];
  if (tags.length) chunks.push(tags.join(' '));
  if (Number.isInteger(betaAi) && betaAi > 0 && Number.isInteger(betaDays) && betaDays > 0) {
    chunks.push(`상담사 테스터 ${betaDays}일 · AI ${betaAi}`);
  } else if (Number.isInteger(betaAi) && betaAi > 0) {
    chunks.push(`AI ${betaAi}`);
  }
...
function parseBetaDaysFromLicenseNote(note) {
  const m = String(note || '').match(BETA_DAYS_NOTE_RE);
  if (!m) return null;
  const n = Number(m[1]);
  if (!Number.isInteger(n) || n < 1) return null;
  return n;
...
function licenseToPublic(row) {
  ...
  return {
    ...
    note,
    memo: extractLicenseMemo(note),
    betaAi: parseBetaAiFromLicenseNote(note),
    betaDays: parseBetaDaysFromLicenseNote(note),
...
async function validateLicenseRow(row, deviceId, jwtPayload = null) {
  ...
  // 기기 1대 고정(license.device_id)은 폐지 — 계정 기기 슬롯(최대 4)으로 대체
  return { ok: true };
}
...
async function requireLifecodeSession(req, res) {
  ...
  try {
    ...
    const check = await validateLicenseRow(row, deviceId || payload.did, payload);
    if (!check.ok) {
      clearSessionCookie(res);
      res.status(401).json({ error: check.error });
      return null;
    }
    if (row.linked_user_id && deviceId) {
      const { assertDeviceAllowed } = require('./account-devices');
      const slot = await assertDeviceAllowed(row.linked_user_id, deviceId);
      if (!slot.ok) {
        res.status(403).json({ error: slot.error || 'device_limit', max: slot.max, devices: slot.devices });
        return null;
      }
    }
    return { payload, row, licenseId: row.id, deviceId: deviceId || payload.did };
  } catch (e) {
...
function registerLifecodeRoutes(app, deps = {}) {
  ...
  registerLifecodeRoutesImpl(app, { getUserIdFromAuth, getProfile, patchProfileFields });
...
function registerLifecodeRoutesImpl(app, deps = {}) {
  const getUserIdFromAuth = deps.getUserIdFromAuth;
  const getProfile = deps.getProfile;
  const patchProfileFields = deps.patchProfileFields;
  ...
  app.post('/api/lifecode/activate', async (req, res) => {
    ...
    let userId = null;
    if (typeof getUserIdFromAuth === 'function') {
      userId = await getUserIdFromAuth(req.headers.authorization);
    }
    if (!userId) {
    ...
    try {
      ...
      if (String(row.product_tier || '').toLowerCase() === 'counselor') {
        res.status(400).json({
          error: 'use_counselor_link',
          message: '상담사 체험권은 계정 연결 페이지에서 등록해 주세요.',
          redirect: '/counselor-trial-link.html',
        });
        return;
      }

      if (row.linked_user_id && row.linked_user_id !== userId) {
        await insertAccessLog(row.id, 'activate_denied_device', deviceId, 'linked other user');
        res.status(403).json({ error: 'license_linked_other' });
        return;
      }

      const { ensureDeviceSlot, MAX_DEVICES_PER_ACCOUNT } = require('./account-devices');
      const slot = await ensureDeviceSlot(userId, deviceId);
      if (!slot.ok) {
        await insertAccessLog(row.id, 'activate_denied_device', deviceId, slot.error);
        res.status(403).json({
          error: slot.error || 'device_limit',
      ...
      // 레거시 컬럼: 최초 기기만 기록(접근 판정에는 미사용)
      if (!row.device_id) {
        patch.device_id = deviceId;
        patch.device_bound_at = nowIso;
      }
      await patchLicense(row.id, patch);
      ...
      if (msg.includes('account_devices')) {
        res.status(503).json({ error: 'devices_table_missing' });
        return;
      }
      res.status(500).json({ error: 'server_error' });
    }
  ...
  app.get('/api/lifecode/session', async (req, res) => {
    ...
    try {
      ...
      if (row.linked_user_id && deviceId) {
        ...
        const slot = await assertDeviceAllowed(row.linked_user_id, deviceId);
        if (!slot.ok) {
          await insertAccessLog(row.id, 'session_denied', deviceId, slot.error);
          res.status(403).json({
            ok: false,
            error: slot.error || 'device_limit',
            devices: slot.devices,
      ...
      await insertAccessLog(row.id, 'session_ok', deviceId || payload.did);
      res.json({
        ok: true,
        licenseId: row.id,
        expiresAt: row.expires_at,
        bound: !!row.linked_user_id,
        productTier: normalizeProductTier(row.product_tier),
        counselorHub: hasCounselorHub(row),
      });
    } catch (e) {
  ...
  /** 로그인 계정에 연결된 베이직 이용권으로 세션 발급 (코드 입력 없이) */
  app.post('/api/lifecode/account-session', async (req, res) => {
    if (!lifecodeConfigured()) {
      res.status(503).json({ ok: false, error: 'server_not_configured' });
      return;
    }
    ...
    try {
      ...
      const { res: sbRes, data } = await sbFetch(
        `/rest/v1/lifecode_licenses?linked_user_id=eq.${encodeURIComponent(userId)}` +
          `&status=eq.active&product_tier=eq.basic&select=id,expires_at,status,product_tier,linked_user_id` +
          `&order=expires_at.desc&limit=10`,
      );
      if (!sbRes.ok) {
        res.status(502).json({ ok: false, error: 'db_error' });
        return;
      }
      const rows = Array.isArray(data) ? data : [];
      const row = rows.find((r) => !isLicenseExpired(r));
      if (!row) {
        res.status(404).json({ ok: false, error: 'no_license' });
        return;
      }
      ...
      const token = signSession(row.id, deviceId);
      setSessionCookie(res, token);
      await insertAccessLog(row.id, 'activate_ok', deviceId, 'account-session');
      ...
      console.error('[lifecode] account-session', e);
      res.status(500).json({ ok: false, error: 'server_error' });
    }
  });
  ...
  app.get('/api/lifecode/admin/licenses', async (req, res) => {
    ...
    try {
      ...
      let path =
        '/rest/v1/lifecode_licenses?select=id,code_hint,status,product_tier,expires_at,device_id,device_bound_at,note,created_at,updated_at,last_seen_at,linked_user_id';
  ...
  app.post('/api/lifecode/admin/licenses', async (req, res) => {
    ...
    try {
      ...
      const betaDays = Number(body.betaDays);
      /* 테스터 = 유료 상담사 체험과 동일(15일·AI25). 5/7은 기존 발급 호환 */
      const isBeta = betaDays === 5 || betaDays === 7 || betaDays === 15;
      const betaAi = isBeta ? (betaDays === 15 ? 25 : 10) : null;
      const product_tier = isBeta ? 'counselor' : normalizeProductTier(body.productTier);

      let expires_at = body.expiresAt || null;
      if (isBeta) {
        const exp = new Date();
      ...
      const betaTag = `[beta_ai:${betaAi}]`;
      const betaDaysTag = `[beta_days:${betaDays}]`;
      let note = body.note || null;
      if (isBeta) {
        const label = `상담사 테스터 ${betaDays}일 · AI ${betaAi}`;
        const rest = String(note || '').trim();
        note = rest
          ? `${betaTag} ${betaDaysTag} ${label} · ${rest}`
          : `${betaTag} ${betaDaysTag} ${label}`;
      }
      ...
      res.status(201).json({
        ...
        note: row.note,
        beta: isBeta,
        betaDays: isBeta ? betaDays : null,
        aiCredits: isBeta ? betaAi : null,
      });
    } catch (e) {
  ...
  app.patch('/api/lifecode/admin/licenses/:id', async (req, res) => {
    ...
    try {
      ...
      if (body.memo !== undefined) {
        const betaAi = parseBetaAiFromLicenseNote(row.note);
        const betaDays = parseBetaDaysFromLicenseNote(row.note);
        fields.note = rebuildLicenseNote({
          betaAi,
          betaDays,
  ...
  /** 만료일 연장 — 라이선스 + (연결 계정이 있으면) plan_active_until 동기화 */
  app.post('/api/lifecode/admin/licenses/:id/extend', async (req, res) => {
    if (!requireAdmin(req, res)) return;
    try {
      const row = await getLicenseById(req.params.id);
      if (!row) {
      ...
      if (body.expiresAt) {
        ...
        if (Number.isNaN(d.getTime())) {
          res.status(400).json({ error: 'invalid_expiresAt' });
          return;
        }
        ...
        res.status(400).json({ error: 'days_or_expiresAt_required' });
        return;
      }

      await patchLicense(row.id, { expires_at: expiresAt });

      let profileSynced = false;
      if (row.linked_user_id && typeof patchProfileFields === 'function') {
        const profilePatch = { plan_active_until: expiresAt };
        if (normalizeProductTier(row.product_tier) === 'counselor') {
          profilePatch.plan = 'professional';
          profilePatch.counselor_trial_license_id = row.id;
        }
        await patchProfileFields(row.linked_user_id, profilePatch);
        profileSynced = true;
      }
      ...
      res.json({
        ok: true,
        license: licenseToPublic(updated),
        profileSynced,
        expiresAt,
      });
    } catch (e) {
      console.error('[lifecode] admin extend', e);
      res.status(500).json({ error: 'server_error' });
    }
  });

  /** 테스터 AI 한도 설정/추가 — note의 [beta_ai:N] */
  app.post('/api/lifecode/admin/licenses/:id/ai-credits', async (req, res) => {
    if (!requireAdmin(req, res)) return;
    try {
      const row = await getLicenseById(req.params.id);
      if (!row) {
      ...
      const body = req.body || {};
      const cur = parseBetaAiFromLicenseNote(row.note) || 0;
      let next = cur;
      if (body.set != null) {
        next = Number(body.set);
      } else if (body.add != null) {
        next = cur + Number(body.add);
      } else {
        res.status(400).json({ error: 'set_or_add_required' });
        return;
      }
      if (!Number.isInteger(next) || next < 1 || next > 500) {
        res.status(400).json({ error: 'invalid_ai_credits' });
        return;
      }
      const betaDays = parseBetaDaysFromLicenseNote(row.note);
      const memo = extractLicenseMemo(row.note);
      const note = rebuildLicenseNote({ betaAi: next, betaDays, memo });
      await patchLicense(row.id, { note });
      try {
        const { invalidateTrialAiCapCache } = require('./ai-usage');
        invalidateTrialAiCapCache(row.id);
      } catch (_) {}
      const updated = await getLicenseById(row.id);
      res.json({
        ok: true,
        license: licenseToPublic(updated),
        betaAi: next,
        previousBetaAi: cur || null,
      });
    } catch (e) {
      console.error('[lifecode] admin ai-credits', e);
      res.status(500).json({ error: 'server_error' });
    }
  });
