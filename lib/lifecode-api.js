...
function registerLifecodeRoutes(app) {
  ...
  app.get('/api/lifecode/admin/licenses', async (req, res) => {
    ...
    try {
      ...
      let path =
        '/rest/v1/lifecode_licenses?select=id,code_hint,status,expires_at,device_id,device_bound_at,note,created_at,updated_at,last_seen_at';
