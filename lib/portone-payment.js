...
function registerPortOneRoutes(app, planBilling, getUserIdFromAuth) {
  ...
  app.post('/api/portone/confirm', async (req, res) => {
    ...
    try {
      ...
      if (!Number.isFinite(total) || !amountMatchesPlan(total, parsed.plan, parsed.cycle)) {
        res.status(400).json({ error: 'Payment amount mismatch' });
        return;
      }

      const applyResult = await planBilling.applyPlanPaymentSuccess(paymentId, paymentId, total);
      if (!applyResult.ok && !applyResult.skipped) {
        res.status(400).json({ error: 'Could not apply plan', reason: applyResult.reason });
        return;
      }

      res.json({ ok: true, plan: applyResult.plan || parsed.plan, skipped: !!applyResult.skipped });
    } catch (e) {
    ...
    try {
      ...
      if (event.type === 'Transaction.Cancelled') {
        ...
        if (paymentId) {
          const parsed = parsePlanOrderId(paymentId);
          if (parsed?.userId) {
            await planBilling.downgradeToFree(parsed.userId, paymentId);
          }
        }
        res.sendStatus(200);
        return;
