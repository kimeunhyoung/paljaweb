...
function registerPortOneRoutes(app, planBilling, getUserIdFromAuth, checkoutStore, profileDeps = {}) {
  ...
  app.post('/api/portone/confirm', async (req, res) => {
    ...
    try {
      ...
      // confirm이 먼저 처리해도 빌링키는 남겨 두어 웹훅/갱신이 동작하게 함
      const billingKey = payment.billingKey || payment.billing_key;
      if (billingKey && planBilling.attachBillingKeyIfMissing) {
        await planBilling.attachBillingKeyIfMissing(ctx.userId, billingKey);
      }
  ...
  app.post('/api/portone/webhook', async (req, res) => {
    ...
    if (!webhookSecretsFromEnv().length) {
      console.error('[portone webhook] PORTONE_WEBHOOK_SECRET 미설정 — 요청 거부');
    ...
    if (!rawBody) {
      console.warn('[portone webhook] missing raw body');
      res.sendStatus(400);
      return;
    }
    ...
    let event;
    try {
      const verified = verifyWithConfiguredSecrets(rawBody, req.headers);
      event = verified.event;
    } catch (e) {
      const reason = e instanceof WebhookVerificationError ? e.reason : 'VERIFY_FAILED';
      console.warn('[portone webhook] signature rejected', reason, e.message || e);
      res.sendStatus(400);
      return;
    }
      ...
      if (event.type === 'Transaction.Paid' || event.type === 'Transaction.VirtualAccountIssued') {
        const paymentId = event.data?.paymentId;
          ...
          try {
            ...
            // 호출 테스트·만료/미존재 결제: PortOne 권장대로 200으로 무시
            const notFound =
              e.status === 404 ||
              e.body?.type === 'PAYMENT_NOT_FOUND' ||
              /payment not found/i.test(String(e.message || ''));
            if (notFound) {
              console.warn('[portone webhook] payment not found (ignored)', paymentId);
              res.sendStatus(200);
              return;
            }
            throw e;
          ...
          if (payment.status === 'PAID') {
            ...
            if (ctx) {
              ...
              const billingKey = payment.billingKey || payment.billing_key;
              if (billingKey) {
                await planBilling.activateSubscriptionOnce(
                  ctx.userId,
                  ctx.plan,
                  ctx.cycle,
                  paymentId,
