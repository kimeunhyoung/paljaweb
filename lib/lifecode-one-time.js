...
const LIFECODE_ONETIME_PRODUCTS = {
  basic30: {
    ...
    title: '베이직 7일 이용권',
    desc: '베이직과 동일 · AI 5크레딧 · 7일. 계속 이용하실 때는 월 9,900원 구독을 추천합니다.',
  },
  counselor30: {
    key: 'counselor30',
    amount: 29900,
...
async function linkLicenseToAccount(license, userId, product, deps = {}) {
  ...
  if (product?.productTier === 'basic') {
    ...
    if (profile?.basic_pass_license_id && profile.basic_pass_license_id !== license.id) {
      return { linked: true, profileSkipped: 'other_basic_pass' };
    }
    await deps.patchProfileFields(userId, {
      plan: 'basic',
      plan_active_until: license.expires_at,
