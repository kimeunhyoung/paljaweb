...
/**
 * URL·문서용 별칭. DB/주문 product_key는 계속 basic30·counselor30 (기존 결제 호환).
 * 키 이름의 "30"은 레거시이며 실제 일수와 무관합니다.
 */
const PRODUCT_ALIASES = {
  basic7: 'basic30',
  counselor15: 'counselor30',
};
...
const LIFECODE_ONETIME_PRODUCTS = {
  ...
  basic30: {
    ...
    productTier: 'basic',
    aiCredits: 5,
    orderName: '라이프코드 베이직 7일 이용권',
    title: '베이직 7일 이용권',
    desc: 'Basic 기능과 동일 · AI 5크레딧 · 7일. 계속 이용하실 때는 월 9,900원 구독을 추천합니다.',
    urlKey: 'basic7',
  },
  // key counselor30 = 레거시 키명 (실제 15일)
  counselor30: {
    key: 'counselor30',
    amount: 29900,
    days: 15,
    ...
    title: '상담사 15일 체험권',
    desc: 'Professional 전 기능 · AI 25크레딧 · 15일. 계속 이용하실 때는 Professional 월 구독을 추천합니다.',
    urlKey: 'counselor15',
  },
};
...
function buildLifecodePaymentId(productKey) {
  ...
  const prefix = PAYMENT_PREFIX[key];
...
function getProduct(productKey) {
  const key = resolveProductKey(productKey);
  return LIFECODE_ONETIME_PRODUCTS[key] || null;
}
...
function publicProducts() {
  return Object.values(LIFECODE_ONETIME_PRODUCTS).map((p) => ({
    key: p.key,
    urlKey: p.urlKey || p.key,
    amount: p.amount,
    days: p.days,
    productTier: p.productTier,
    aiCredits: p.aiCredits || null,
    title: p.title,
    desc: p.desc,
...
/** 남은 이용권이 있으면 그 끝에서부터, 없으면 지금부터 days 연장 */
function computeExtendedExpiry(currentUntilIso, days) {
  const dayCount = Math.max(0, Number(days) || 0);
  const now = Date.now();
  const curMs = currentUntilIso ? new Date(currentUntilIso).getTime() : 0;
  const base = Number.isFinite(curMs) && curMs > now ? curMs : now;
  return new Date(base + dayCount * 24 * 60 * 60 * 1000).toISOString();
}

/** pending → paid 원자적 선점. 이미 처리 중/완료면 null */
async function claimOrderPaid(paymentId) {
  const { res, data } = await sbFetch(
    `/rest/v1/lifecode_orders?payment_id=eq.${encodeURIComponent(paymentId)}&status=eq.pending`,
    {
      method: 'PATCH',
      headersExtra: { Prefer: 'return=representation' },
      body: JSON.stringify({
  ...
  if (!res.ok) throw new Error(`lifecode_orders claim ${res.status}: ${JSON.stringify(data)}`);
  return Array.isArray(data) && data[0] ? data[0] : null;
}

async function findLicenseByPaymentNote(paymentId) {
  const pid = String(paymentId || '').trim();
  if (!pid) return null;
  const { res, data } = await sbFetch(
    `/rest/v1/lifecode_licenses?note=like.${encodeURIComponent(`*${pid}*`)}&limit=1`,
  );
 ...
 * basic → profiles.plan=basic (7일·AI 5), counselor → professional 체험 (AI 25)
 * 이미 같은 종류의 이용권이 있으면 만료일을 이어서 연장합니다.
 */
async function linkLicenseToAccount(license, userId, product, deps = {}) {
  ...
  if (typeof deps.patchProfileFields !== 'function') {
    return { linked: true, expiresAt: license.expires_at || null };
  }

  const profile = typeof deps.getProfile === 'function' ? await deps.getProfile(userId) : null;
  if (profile?.toss_billing_key) {
    return { linked: true, profileSkipped: 'has_subscription', expiresAt: license.expires_at || null };
  }

  const days = Math.max(1, Number(product?.days) || 7);

  if (product?.productTier === 'counselor') {
    const hadOther =
      !!(profile?.counselor_trial_license_id) &&
      profile.counselor_trial_license_id !== license.id;
    const newExpires = computeExtendedExpiry(profile?.plan_active_until, days);
    await patchLicense(license.id, { expires_at: newExpires });
    await deps.patchProfileFields(userId, {
      plan: 'professional',
      plan_active_until: newExpires,
      counselor_trial_license_id: license.id,
    });
    return {
      linked: true,
      counselorActivated: true,
      extended: hadOther || !!(profile?.plan_active_until && new Date(profile.plan_active_until) > new Date()),
  ...
  if (product?.productTier === 'basic') {
    ...
    if ((rank[curPlan] || 0) > 1) {
      return { linked: true, profileSkipped: 'higher_plan', expiresAt: license.expires_at || null };
    }
    const hadOther =
      !!(profile?.basic_pass_license_id) &&
      profile.basic_pass_license_id !== license.id;
    const newExpires = computeExtendedExpiry(profile?.plan_active_until, days);
    await patchLicense(license.id, { expires_at: newExpires });
    await deps.patchProfileFields(userId, {
      plan: 'basic',
      plan_active_until: newExpires,
      basic_pass_license_id: license.id,
    });
    return {
      linked: true,
      basicActivated: true,
 ...
 * pending→paid 선점 후 발급. 이미 paid면 라이선스만 보강.
 */
async function fulfillLifecodeOrder(paymentId, deps = {}) {
  ...
  const buildResult = (lic, linkInfo, alreadyFulfilled) => ({
    ok: true,
    alreadyFulfilled: !!alreadyFulfilled,
    expiresAt: linkInfo?.expiresAt || lic?.expires_at || null,
    productTier: lic?.product_tier || getProduct(order.product_key)?.productTier || null,
    counselorHub: String(lic?.product_tier || getProduct(order.product_key)?.productTier || '') === 'counselor',
    linkedUserId: order.user_id || lic?.linked_user_id || null,
    accountLinked: true,
    counselorActivated: !!linkInfo?.counselorActivated,
    basicActivated: !!linkInfo?.basicActivated,
    extended: !!linkInfo?.extended,
    profileSkipped: linkInfo?.profileSkipped || null,
  });

  if (order.status === 'paid' && order.license_id) {
    const lic = await getLicenseById(order.license_id);
    let linkInfo = null;
    // 라이선스만 있고 계정 미연결인 경우만 연결 (기간 재연장 금지)
    if (order.user_id && lic && !lic.linked_user_id) {
      const product = getProduct(order.product_key);
      linkInfo = await linkLicenseToAccount(lic, order.user_id, product, deps);
    }
    return buildResult(lic, linkInfo, true);
  }
  ...
  // 아직 pending이면 선점. 이미 다른 워커가 paid로 바꿨으면 라이선스 생성은 맡김.
  if (order.status === 'pending') {
    const claimed = await claimOrderPaid(paymentId);
    if (!claimed) {
      for (let i = 0; i < 12; i += 1) {
        ...
        if (order?.status === 'paid' && order.license_id) {
          const lic = await getLicenseById(order.license_id);
          return buildResult(lic, null, true);
        }
        const existing = await findLicenseByPaymentNote(paymentId);
        if (existing) {
          await patchOrder(paymentId, { license_id: existing.id });
          return buildResult(existing, null, true);
        }
      }
      order = await getOrder(paymentId);
      ...
      // 타임아웃 후에도 라이선스 없으면 아래에서 복구 발급
    } else {
      order = claimed;
    }
  }

  let lic = order.license_id ? await getLicenseById(order.license_id) : null;
  if (!lic) {
    lic = await findLicenseByPaymentNote(paymentId);
  }
  if (!lic) {
  ...
  await patchOrder(paymentId, { license_id: lic.id });
  order = { ...order, license_id: lic.id, status: 'paid' };

  let linkInfo = { linked: false };
  if (order.user_id) {
    // 이미 이 라이선스가 연결된 재진입이면 연장 재적용하지 않음
    if (lic.linked_user_id && String(lic.linked_user_id) === String(order.user_id)) {
      linkInfo = { linked: true, expiresAt: lic.expires_at || null };
    } else {
      linkInfo = await linkLicenseToAccount(lic, order.user_id, product, deps);
    }
  }

  return buildResult(lic, linkInfo, false);
}
...
function registerLifecodeOneTimeRoutes(app, {
  ...
  app.post('/api/lifecode/checkout/prepare', async (req, res) => {
    ...
    const productKeyRaw = String(req.body?.productKey || 'basic30');
      ...
      if (typeof getProfile === 'function') {
        ...
        if (profile?.toss_billing_key) {
          res.status(409).json({
            ...
            message: '이미 월 구독 이용 중입니다. 건별 이용권 대신 구독 플랜을 이용해 주세요.',
          });
          return;
        }
        if (product.productTier === 'basic') {
          ...
          if ((rank[curPlan] || 0) > 1) {
            res.status(409).json({
              error: 'higher_plan',
              message: '이미 Plus 이상 플랜을 이용 중입니다. 베이직 이용권은 필요하지 않습니다.',
            });
            return;
          }
        }
