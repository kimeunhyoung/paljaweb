...
async function isProfessionalUser(userId) {
  ...
  if (!profile || !isCounselorPlan(profile)) return false;
...
async function getTodayScheduleForCounselor(counselorId, today) {
  ...
  try {
    const events = await sbGet(
      'counselor_events',
      `counselor_id=eq.${encodeURIComponent(counselorId)}&event_date=eq.${encodeURIComponent(today)}&select=id,title,event_time,event_kind`,
    );
    events.forEach((ev) => {
      const key = 'e|' + ev.id;
      if (seen.has(key)) return;
