...
async function sendTestPushToUser(userId) {
  ...
  if (!(await isProfessionalUser(userId))) {
    return { ok: false, error: 'professional_required' };
  }
  const subs = await sbGet(
    'counselor_push_subscriptions',
    `counselor_id=eq.${encodeURIComponent(userId)}&morning_enabled=eq.true&select=id,counselor_id,endpoint,p256dh,auth_key`,
  );
  if (!subs.length) {
    return { ok: false, error: 'no_subscription' };
  }
  const today = todayKstYmd();
  const items = await getTodayScheduleForCounselor(userId, today);
  const preview = items.length
    ? items.slice(0, 3).map((x) => x.name).join(', ')
    : '(오늘 등록된 상담·일정 없음 — 테스트 알림입니다)';
  const payload = {
    title: '팔자연구소 일정 알림 테스트',
    body: preview,
    url: '/counselor.html',
    tag: 'counselor-push-test-' + Date.now(),
  };
  let sent = 0;
  let failed = 0;
  ...
  return { ok: sent > 0, sent, failed, today, scheduleCount: items.length };
}
...
function registerCounselorPushRoutes(app, getUserIdFromAuth) {
  ...
  app.post('/api/counselor/push/test', async (req, res) => {
    const userId = await getUserIdFromAuth(req.headers.authorization);
    if (!userId) {
      res.status(401).json({ error: 'Unauthorized' });
      return;
    ...
    try {
      ...
      console.error('[counselor/push/test]', e);
...
let lastScheduledHourKey = null;

/** Render 등 외부 크론 없이도 매 정시(한국)에 1회 발송 */
function startCounselorPushScheduler() {
  if (String(process.env.DISABLE_PUSH_SCHEDULER || '').toLowerCase() === 'true') {
    console.log('[push-scheduler] disabled (DISABLE_PUSH_SCHEDULER=true)');
    return;
  }
  if (!vapidConfigured()) {
    console.warn('[push-scheduler] VAPID not set — scheduler skipped');
    return;
  }
  ...
  const tick = async () => {
    ...
    const key = `${today}T${hour}`;
    if (lastScheduledHourKey === key) return;
    lastScheduledHourKey = key;
    try {
      const result = await runMorningPushJob();
      console.log('[push-scheduler]', JSON.stringify(result));
    } catch (e) {
      console.error('[push-scheduler]', e);
      lastScheduledHourKey = null;
    }
  };

  setInterval(tick, 60_000);
  tick();
  console.log('[push-scheduler] started (KST hourly, in-process)');
}
