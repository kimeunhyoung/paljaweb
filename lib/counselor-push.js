...
async function getTodayScheduleForCounselor(counselorId, today) {
  ...
  try {
    const events = await sbGet(
      'counselor_events',
      `counselor_id=eq.${encodeURIComponent(counselorId)}&event_date=eq.${encodeURIComponent(today)}&select=title`,
    );
    events.forEach((ev) => {
...
module.exports = { registerCounselorPushRoutes, runMorningPushJob, todayKstYmd, currentKstHour };
