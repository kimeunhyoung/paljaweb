...
const NAVER_SETUP_ERROR =
  'Render 환경변수 NAVER_CLIENT_ID, NAVER_CLIENT_SECRET을 추가한 뒤 Save → 재배포해 주세요.';

function naverConfigured() {
  return !!(process.env.NAVER_CLIENT_ID && process.env.NAVER_CLIENT_SECRET);
}

function naverConfigStatus() {
  return {
    ...
    redirectUri: process.env.NAVER_REDIRECT_URI || 'https://8code.kr/api/auth/naver/callback',
  };
}
...
function registerNaverAuthRoutes(app) {
  app.get('/api/auth/naver/status', (_req, res) => {
    res.json(naverConfigStatus());
  });

  app.get('/api/auth/naver', (req, res) => {
    if (!naverConfigured()) {
      res.redirect('/login.html?error=' + encodeURIComponent(NAVER_SETUP_ERROR));
      return;
    }
...
module.exports = { registerNaverAuthRoutes, naverConfigured, naverConfigStatus };
