...
function verifySignupWebhookSecret(req) {
  ...
  // Supabase / Edge / 프록시 조합에 따라 헤더 키가 달라질 수 있어 후보들을 모두 허용합니다.
  const headerCandidates = {
    'x-signup-secret': req.headers['x-signup-secret'],
    x_signup_secret: req.headers['x_signup_secret'],
    xsignupsecret: req.headers['xsignupsecret'],
    authorization: req.headers.authorization,
  };

  // Supabase가 Authorization에 'Bearer <secret>' 형태 또는 '<secret>' 형태로 넣을 수 있어 둘 다 허용합니다.
  ...
  const xSecret =
    headerCandidates['x-signup-secret'] ||
    headerCandidates.x_signup_secret ||
    headerCandidates.xsignupsecret ||
    token;

  // 헤더가 안 들어오는 케이스를 빠르게 찾기 위한 디버그 로그
  if (!xSecret) {
    console.warn('[signup-notify] webhook secret candidate missing', {
      hasAuthorization: !!req.headers.authorization,
      hasXSignupSecret: !!req.headers['x-signup-secret'],
      hasXSignupSecretUnderscore: !!req.headers['x_signup_secret'],
      allHeaderKeysCount: Object.keys(req.headers || {}).length,
    });
  }
...
function registerSignupNotifyRoutes(app) {
  app.post('/api/webhooks/signup', async (req, res) => {
    ...
    console.log('[webhooks/signup] hit', {
      ...
      hasBody: !!req.body,
      headerKeysPreview: Object.keys(req.headers || {}).slice(0, 25),
      authorizationPreview: req.headers.authorization ? String(req.headers.authorization).slice(0, 30) : null,
      xSignupSecretPreview: req.headers['x-signup-secret']
        ? String(req.headers['x-signup-secret']).slice(0, 30)
        : null,
    });
