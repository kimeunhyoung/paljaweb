...
function isPaidProfessional(profile) {
  ...
  return (
    ...
    !!profile.professional_payment_key &&
...
const ERROR_MESSAGES = {
  ...
  not_counselor_license: '상담사 15일 체험권 코드가 아닙니다.',
  ...
  device_mismatch: '이 코드는 다른 기기에 등록되어 있습니다. 결제·연결했던 기기에서 이용해 주세요.',
  other_trial: '다른 체험 코드가 이미 이 계정에 연결되어 있습니다.',
  trial_device_mismatch: '체험권은 최초 연결한 기기 1대에서만 이용할 수 있습니다.',
...
async function linkCounselorTrial(userId, code, deviceId, deps) {
  ...
  if (isLicenseExpired(license)) return { ok: false, error: 'expired' };
  ...
  if (profile.counselor_trial_device_id && profile.counselor_trial_device_id !== deviceId) {
    return { ok: false, error: 'trial_device_mismatch' };
  }
  ...
  if (!license.device_id) {
    licensePatch.device_id = deviceId;
    licensePatch.device_bound_at = nowIso;
  }
  try {
    ...
    await deps.patchProfileFields(userId, {
      ...
      counselor_trial_device_id: deviceId,
    });
  } catch (e) {
...
function trialStatusForProfile(profile, deviceId) {
  ...
  const bound = profile.counselor_trial_device_id || null;
  const deviceOk = !bound || !deviceId || bound === deviceId;
  return {
    isTrial: true,
    active,
...
function registerCounselorTrialRoutes(app, deps) {
  ...
  app.post('/api/counselor-trial/link', async (req, res) => {
      ...
      const status =
        result.error === 'unauthorized'
          ...
          : ['invalid_code', 'not_counselor_license', 'invalid_device'].includes(result.error)
            ? 400
            : ['device_mismatch', 'trial_device_mismatch', 'license_linked_other', 'other_trial'].includes(
                result.error,
              )
              ? 403
    ...
    res.json({
      ...
      message: 'Professional 15일 체험이 연결되었습니다. 상담사 허브를 이용해 보세요.',
    });
  });
  ...
  app.get('/api/counselor-trial/status', async (req, res) => {
    ...
    const trial = trialStatusForProfile(profile, deviceId);
    res.json({
      plan: profile.plan,
